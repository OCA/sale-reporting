...
class sale_condition_text_webkit(osv.osv):
	"""add info condition in the sale"""
	_name = "sale.condition_text_webkit"
	_description = "Sale condition text"
...
class sale_order(osv.osv):
	...
	def set_comment_webkit(self, cr,uid,id,commentid):
		if not commentid :
			return {}
		cond = self.pool.get('sale.condition_text_webkit').browse(
			cr,uid,commentid,{})
		...
		if cond :
			...
			try :
				...
				lang = 'en_EN'
			res_trans = self.get_trans_webkit(cr, uid, 'sale.condition_text_webkit,text', commentid, lang )
			if not res_trans :
				res_trans = text
	...
	def set_note_webkit(self, cr,uid,id,commentid):
		...
		if cond :
			...
			res_trans = self.get_trans_webkit(cr, uid, 
				'sale.condition_text_webkit,text', commentid, lang )
			if not res_trans :
				res_trans = text
...
#		return tmp_id
	...
	_columns = {
		'text_condition1_webkit': fields.many2one('sale.condition_text_webkit', 'Header'),
		'text_condition2_webkit': fields.many2one('sale.condition_text_webkit', 'Footer'),
